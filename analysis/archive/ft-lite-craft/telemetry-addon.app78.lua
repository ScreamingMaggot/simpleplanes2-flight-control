...
function ap_button(id)
	...
	elseif id == 8 then
		-- SC-5 app74 诊断（只读通路对照用，不参与任何律）：起落架有两条可能的驱动通路，
		-- ①机体控制轴（封版 Lua 走这条，app18"带轮=能量死刑"是它物理生效的实锤）；
		-- ②零件 InputController.State 的 FT 输入位（v0.9~v0.12b 四种写法、含纯内置量两值
		-- 自证，全部零阻力台阶 ⇒ 判定这条通路对 GearLeg/GearBay 不生效）。
		-- 按 8 一次=轴置 1（放下），再按一次=交还（ReleaseInput），日志打 LGEAR 行供事后核对。
		if not lgear_ovr then
			local ok = pcall(function() craft.Controls:OverrideInput("LandingGear", 1) end)
			lgear_ovr = true
			print(string.format("LGEAR override(1) ok=%s readback=%s", tostring(ok),
				tostring(craft.Controls.LandingGearDown)))
		else
			pcall(function() craft.Controls:ReleaseInput("LandingGear") end)
			lgear_ovr = false
			print(string.format("LGEAR released readback=%s", tostring(craft.Controls.LandingGearDown)))
		end
	elseif id == 9 then
...
-- ==== SC-5 FT-Lite 探杆（app54 定罪：FT 律零遥测，判案靠猜=违背读数据纪律）。
-- 每局 5 Hz 打 `ALARM,cid,t,gear,corridor,g,SD,LT`：gear=起落架放下态（v0.11 起起落架=短五边
-- 构型（SD<2500 才放轮），不再当 ARM 灯；A7 一列 app55 证实 Lua 代理读不到已删）；
-- 后四列=Lua 侧镜像 ft_autoland_patch.py **v0.11** 选择器（窗=|LT|<max(900,2·IAS²/13.5) ∧
-- −200<SD<15000，选 SD 最小者，SD=−99999=表外）。镜像与 FT 同表同式，差值即烘焙错位。
-- ⚠ 改 FT 面板的几何/闸门时必须同步改这里，否则判案读的是旧镜（app71 之后已同步一次）。
ft_probe_fr = 0
function ftlite_probe()
	ft_probe_fr = ft_probe_fr + 1
	if math.fmod(ft_probe_fr, 8) ~= 0 then return end
	local gd = -1
	pcall(function() gd = craft.Controls.LandingGearDown and 1 or 0 end)
	-- 镜像 FT 面板 v0.11（逐字同式，差值即烘焙错位）：闸门=SD∈(−200,15000) ∧
	-- |LT| < max(900, 2·IAS²/13.5)；选择=已建立走廊中**最近者**（SD 最小，平手取表序先）；
	-- 无解=哨兵 −99999。旧版镜像 v0.4（1.15|LT|≤r55 + 黏滞 + 表序）已作废，别再照它判案。
	local bn, bs, bl = "-", -99999, -99999
	for i, e in ipairs(RWYS) do
		local r = e.hdg * math.pi / 180
		local dla, dlo = e.lat - craft.Latitude, e.lon - craft.Longitude
		local s = dla * math.cos(r) + dlo * math.sin(r) + LG_TGT_IN   -- SD（瞄准点系）
		local l = dla * math.sin(r) - dlo * math.cos(r)               -- LT 右正
		if s > -200 and s < 15000 and math.abs(l) < math.max(900, 2 * craft.IAS * craft.IAS / 13.5)
			and (bs == -99999 or s < bs) then bs, bl, bn = s, l, e.n end
	end
	print(string.format("ALARM,%d,%.3f,%d,%s,%d,%.0f,%.0f",
		_cidn, craft.Time, gd, bn, bs and 1 or 0, bs, bl))
end
